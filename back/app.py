...
def calculate_shortest_path():
    ...
    source = (so[0], so[1])
    target = (de[0], de[1])
    graph = ox.graph_from_place('Boumerdes , Algeria', network_type='drive')
    ...
    fig , ax = ox.plot_graph_route(graph, shortest_path, show=False, close=False, edge_color='white', edge_linewidth=1, node_size=0)
